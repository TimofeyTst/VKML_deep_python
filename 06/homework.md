...
`python server.py -w 10 -k 7 --host 0.0.0.0 --port 8080` (сервер использует 10 воркеров для обкачки и отправляет клиенту топ-7 частых слов)

Для debug режима дополнительно указываем флаг --debug:
```bash
    python3 server.py -w 2 -k 2 --debug
```
...
Для debug режима дополнительно указываем флаг --debug:
```bash
    python3 client.py 10 urls.txt --debug
```
